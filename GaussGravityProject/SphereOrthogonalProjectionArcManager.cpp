...
SphereOrthogonalProjectionArcManager::SphereOrthogonalProjectionArcManager(SphereOrthogonalProjectionScene* _scene)
    ...
    ellipseLine = scene->addLine(0, 0, scene->getProjectionCircleRadius(), 0, QPen(Qt::white));
    ellipseLine->setTransformOriginPoint(0, 0);
    ellipseLine->setVisible(false);
}
                                             ...
                                             bool isClosed)
{
    ...
    if (vacantArcList.empty()) {
        ...
        arc->foregroundItem->setTransformOriginPoint(0, 0);
        arc->backgroundItem->setTransformOriginPoint(0, 0);
...
void SphereOrthogonalProjectionArcManager::drawProjection(Arc *arc) {
    ...
    double sphereRadius = scene->getProjectionCircleRadius();
    ...
    if (arc->isClosed) {
        ...
        foregroundPath.moveTo(sphereRadius, 0);
        arc->foregroundItem->setPen(arc->foregroundPen);
        foregroundPath.arcTo(-a, -b, 2 * a, 2 * b, 0, -180 * sgn);

        backgroundPath.moveTo(sphereRadius, 0);
        arc->backgroundItem->setPen(arc->backgroundPen);
        backgroundPath.arcTo(-a, -b, 2 * a, 2 * b, 0, 180 * sgn);
        ...
        double startAngle = sign(cross(intersectionGuideProjection, startProjection))
                            * qRadiansToDegrees((double)arc->startPlanePoint
        ...
        foregroundPath.arcMoveTo(-a, -b, 2 * a, 2 * b, startAngle);
        arc->foregroundItem->setPen(arc->foregroundPen);
        foregroundPath.arcTo(-a, -b, 2 * a, 2 * b, startAngle, sweepLength);
    }
